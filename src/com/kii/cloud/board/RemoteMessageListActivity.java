...
public class RemoteMessageListActivity extends ListActivity {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == MENU_DELETE) {
            ...
            final int idx = mCursor.getColumnIndex(TopicCache.URI);
            final String uriString = mCursor.getString(idx);
            final Uri uri = Uri.parse(uriString);
            KiiQuery query = new KiiQuery();
            query.setWhere(KQExp.equals(Message.PROPERTY_TOPIC, uriString));
            KiiObject
                    .deleteQuery(null, KiiBoardClient.CONTAINER_MESSAGE, query);
            KiiObject obj = new KiiObject(uri);
            obj.delete(new KiiObjectCallBack() {
                @Override
                public void onDeleteCompleted(int token, boolean success,
                        Exception exception) {
                    if (success) {
                        Uri deleteUri = ContentUris.withAppendedId(TopicCache.CONTENT_URI,
                                info.id);
    ...
    private void updateLocalDB(List<KiiObject> entities) {
        ...
        for (KiiObject obj : entities) {
            ...
            values.put(TopicCache.URI, obj.toUri().toString());
